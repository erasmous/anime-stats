...
class RecentlyAdded extends React.Component<any, any> {
  ...
  formatResults(result: []) {
    ...
    if (data.length) slides[counter++] = data;
  ...
  createSlides(formattedResult: any) {
    ...
    for (let i = 1; i < formattedResult.length; i++) {
      ...
      for (let x = 0; x < rows.length; x++) {
        ...
        const element = (
          <Link to={"anime/" + entry.mal_id} key={x}>
            <div className="carousel-column">
              <div className="featured-wrapper">
                <span className="rating">
                  <span className="star">&#10004;</span> New update
                </span>
                <img className="featured-image" src={image} alt="images" />
      ...
      slides.push(
        <Slide key={i} index={i}>
          <div className="recently-added">{elements}</div>
        </Slide>
      );
    }
      ...
      <div>
        {this.state.error ? (
          ...
          >
            ...
            <Slider>{this.state.slides}</Slider>
          </CarouselProvider>
        )}
      </div>
    );
